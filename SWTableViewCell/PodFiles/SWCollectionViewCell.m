...
@interface SWCollectionViewCell () <UIScrollViewDelegate,  UIGestureRecognizerDelegate>
...
- (void)initializer {
    ...
    for (NSUInteger i = 0; i < 2; ++i) {
        ...
        NSLayoutConstraint *widthConstraint = [NSLayoutConstraint constraintWithItem:buttonView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationLessThanOrEqual toItem:self.contentView attribute:NSLayoutAttributeWidth multiplier:1.0 constant:-kUtilityButtonWidthDefault];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:kCollectionViewPanState] && object == _collectionViewPanGestureRecognizer) {
        if (_collectionViewPanGestureRecognizer.state == UIGestureRecognizerStateBegan) {
            CGPoint locationInCollectionView = [_collectionViewPanGestureRecognizer locationInView:_containingCollectionView];
            
            BOOL inCurrentCell = CGRectContainsPoint(self.frame, locationInCollectionView);
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    if (_cellState != kCellStateCenter) {
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)]) {
            for (SWCollectionViewCell *cell in [self.containingCollectionView visibleCells]) {
                if (cell != self && [cell isKindOfClass:[SWCollectionViewCell class]] && [self.delegate swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:cell]) {
